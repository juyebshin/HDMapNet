...
def visualize(writer: SummaryWriter, title, imgs: torch.Tensor, dt_mask: torch.Tensor, vt_mask: torch.Tensor, 
    ...
    if matches is not None and positions is not None and masks is not None:
        ...
        # matches_gt: b c N+1 N+1
        # xbound: []
        # ybound: []
        matches = matches[0].detach().cpu().float().numpy() # c N+1 N+1
        positions = positions[0].detach().cpu().float().numpy()[..., :-1] # c N 2
        masks = masks[0].detach().cpu().int().numpy().squeeze(-1) # c N
        # attentions = attentions[0].detach().cpu().float().numpy()[-1] # [H, N, N] attention of the last layer
        bins = np.ones([masks.shape[0]]).reshape(masks.shape[0], 1) # 3 1
        ...
        for ci, (cmatches, cpositions, cmasks_bins) in enumerate(zip(matches, positions, masks_bins)):
            cpositions_valid = cpositions[masks[ci] == 1] # M 2
            cmatches = cmatches[cmasks_bins == 1][:, cmasks_bins == 1] # masked M+1 M+1
            matches_nodust = cmatches[:-1, :-1] # M M
            rows, cols = np.where(matches_nodust > args.match_threshold)
            for row, col in zip(rows, cols):
                plt.plot([cpositions_valid[row, 0], cpositions_valid[col, 0]], [cpositions_valid[row, 1], cpositions_valid[col, 1]], 'o-', c=cm.jet(matches_nodust[row, col]), linewidth=0.5, markersize=1.0)
        ...
        writer.add_image(f'{title}/match_pred', colorise(matches, 'hot', 0.0, 1.0), step, dataformats='NHWC')
        ...
        for ci, (cmatches_gt, cpositions, cmasks_bins) in enumerate(zip(matches_gt, positions, masks_bins)):
            cpositions_valid = cpositions[masks[ci] == 1] # M 2
            cmatches_gt = cmatches_gt[cmasks_bins == 1][:, cmasks_bins == 1] # M+1 M+1
            # matches_gt = matches_gt[:, :-1] # [M, M]
            matches_idx = cmatches_gt.argmax(-1) if len(matches_gt) > 0 else None # [M, ]
            
            plt.scatter(cpositions_valid[:, 0], cpositions_valid[:, 1], s=0.5, c='b', vmin=0.0, vmax=1.0)
            for i, pos in enumerate(cpositions_valid): # [3,]
                if matches_idx is not None:
                    match = matches_idx[i]
                    if cmatches_gt[i, match] == 1.0 and match < len(cmatches_gt)-1: # less then N
                        # plt.plot([pos[0], positions_valid[match][0]], [pos[1], positions_valid[match][1]], '-', color=colorise(matches_gt[i, match], 'jet', 0.0, 1.0))
                        plt.quiver(pos[0], pos[1], cpositions_valid[match][0] - pos[0], cpositions_valid[match][1] - pos[1], 
                                color=cm.jet(cmatches_gt[i, match]), scale_units='xy', angles='xy', scale=1)
        ...
        writer.add_image(f'{title}/match_gt', colorise(matches_gt, 'hot', 0.0, 1.0), step, dataformats='NHWC')
        ...
        for ci, (cmatches, cpositions, cmasks_bins) in enumerate(zip(matches, positions, masks_bins)):
            ...
            rows, cols = np.where(matches_nodust > args.match_threshold)
            for row, col in zip(rows, cols):
                plt.plot([cpositions_valid[row, 0], cpositions_valid[col, 0]], [cpositions_valid[row, 1], cpositions_valid[col, 1]], 'o-', c=colors_plt[ci], linewidth=0.5, markersize=1.0)
...
def eval_iou(model, val_loader, args, writer=None, step=None, vis_interval=0, is_master=False):
    ...
    with torch.no_grad():
        for imgs, trans, rots, intrins, post_trans, post_rots, lidar_data, lidar_mask, car_trans, yaw_pitch_roll, semantic_gt, instance_gt, distance_gt, vertex_gt, vectors_gt in tqdm.tqdm(val_loader):
            ...
            heatmap = vertex.sigmoid() # b c 25 50
            heatmap[heatmap < args.vertex_threshold] = 0.0
            matches = matches.exp() # b c N+1 N+1
            ...
            if writer is not None and vis_interval > 0:
                if counter % vis_interval == 0 and is_master:                
                        ...
                        visualize(writer, 'eval', imgs, distance_gt, vertex_gt.max(2)[0], vectors_gt, matches_gt, None, distance, heatmap, matches, positions, None, masks, step, args)
