...
def preprocess_map(vectors, patch_size, canvas_size, num_classes, thickness, angle_class, cell_size=8):
    ...
    vertex_masks = np.stack(vertex_masks) # 3, 200, 400
    ...
    for c, vertex_mask in enumerate(vertex_masks): # for every class, vertex_mask: 64, Hc, Wc
        ...
        # N == len(rows) == len(cols)
        if len(rows):
            multi_vertex = vertex_masks[c, :, [row for row in rows], [col for col in cols]] # N, 64
    ...
    dust = np.zeros_like(vertex_sum, dtype='uint8') # 3, Hc, Wc
    dust[vertex_sum == 0] = 1
