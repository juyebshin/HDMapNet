...
class DiscriminativeLoss(nn.Module):
    ...
    def forward(self, embedding, seg_gt):
        ...
        for b in range(bs):
            ...
            labels = torch.unique(seg_gt_b) # [N]
            labels = labels[labels != 0] # exclude background
            num_lanes = len(labels) # n_lane
            if num_lanes == 0:
            ...
            for lane_idx in labels:
                seg_mask_i = (seg_gt_b == lane_idx) # [H, W]
                if not seg_mask_i.any():
                    continue
                embedding_i = embedding_b[:, seg_mask_i] # [embed_dim, N]
            ...
            if num_lanes > 1:
                centroid_mean1 = centroid_mean.reshape(-1, 1, self.embed_dim) # [n_lane, 1, embed_dim]
                centroid_mean2 = centroid_mean.reshape(1, -1, self.embed_dim) # [1, n_lane, embed_dim]
